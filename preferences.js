...
firebase.auth().onAuthStateChanged(async function(user) {
    ...
    submitPreferences.addEventListener('click', async function(event) {
        ...
        let f = document.getElementById("matching_factor")
        let matchingFactor = f.options[f.selectedIndex].text

        let response = await fetch('/.netlify/functions/create_preferences',{
            method: 'POST',
            body: JSON.stringify({
                userID: user.uid,
        ...
        if (matchingFactor == "- Select -"){
            alert("Please select a factor to be matched on")
        } else {
            seeMatches()
        }
    }) // end click listener
